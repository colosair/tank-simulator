...
public class DAEJEON4_BAEHANJIN1 {
	private static String NICKNAME = "대전4반_배한진";
		...
		while (!queue.isEmpty()) {
			...
			for (int d = 0; d < 4; d++) {
				...
				if (nr >= 0 && nr < mapData.length && nc >= 0 && nc < mapData.length && !visited[nr][nc]
					...
					queue.add(new Node1(nr, nc, temp));
...
class Node1 {
	...
	public Node1(int r, int c, ArrayList<String> path) {
		this.r = r;
		this.c = c;
		this.path = path;
	}
